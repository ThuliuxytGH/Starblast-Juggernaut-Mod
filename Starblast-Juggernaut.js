...
const gameOptions = { 
  ...
  */
  ...
  gameImages: {
    ...
    "gotofight": {
      image: "ThuliuxytGH/Juggernaut-textures-map/main/background-images/gottofight.png",
      info: {
        size: {x: 50, y: 14},
        depth: -1,
        x: 0,
...
var functions = {
  usage: {
    ...
    spawn: function(ship) { // manage spawn
      ...
      const type = this.random(gameOptions.teams[1].ships);
      if (ship.custom.initialized) {
        this.setShip(1, ship.type, type, {x:0, y:-410});
        return;
      }
      this.setShip(1, ship, type, {x:0, y:-410});
      ship.custom.isJuggernaut = false;
      ship.custom.initialized = true;
    },
    outOfSpawn: function(ship) {
      ship.set({x:ship.x, y:-360});
      functions.buttons._switch.toggle(ship, false);
    },
    checkOutOfSpawn: function(ship) {
      return (ship.x < 22 && ship.x > -22 && ship.y < -380 && ship.y > -390)
    }
  },
  buttons: {
    _switch: {
    ...
    if (game.step % 20 === 0) {
      try {
        ...
        .forEach(ship => { // loop for players
          ...
          if (functions.usage.checkOutOfSpawn(ship)) {
  ...
  initializeImages: function() {
    ...
    for (let key of Object.keys(images)) {
      ...
      game.setObject({
        ...
        scale: { x: info.size.x, y: info.size.y, z: 0 },
